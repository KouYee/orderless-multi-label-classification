from data import COCOMultiLabel, categories
...
from data import category_dict_sequential, category_dict_sequential_inv
...
if __name__ == "__main__":
    ...
    if swa_params:
        ...
        if not args.test_model:
            scheduler_decoder = CyclicalLR(decoder_optimizer,swa_params['lr_high'],swa_params['lr_low'],swa_params['cycle_length'])
            if finetune_encoder:
                scheduler_encoder = CyclicalLR(encoder_optimizer,swa_params['lr_high'] * swa_coeff,swa_params['lr_low'] * swa_coeff,swa_params['cycle_length'])
        decoder_swa = Decoder(args.hidden_size, args.embed_size,args.attention_size, args.dropout).to('cuda')
    ...
    dataset = COCOMultiLabel(train=True,classification=False,image_path=args.image_path,sort_by_freq=args.sort_by_freq)
    dataset_val = COCOMultiLabel(train=False,classification=False,image_path=args.image_path,sort_by_freq=args.sort_by_freq)
    dataloader = DataLoader(dataset,batch_size=args.batch_size,num_workers=args.num_workers,
        pin_memory=True,shuffle=True,drop_last=False,collate_fn=my_collate)
    dataloader_val = DataLoader(dataset_val,batch_size=args.batch_size,num_workers=args.num_workers,
                                pin_memory=True,shuffle=False,drop_last=False,collate_fn=my_collate)
    ...
    for epoch in range(resume, args.epochs):
        ...
        if training:
            ...
            for i, batch in enumerate(dataloader):
                ...
                # training accuracy
                if i % 50 == 0:
                    preds_train, labels_train = convert_to_array(scores, targets,label_lengths_sorted)
                    _, _, f1, _ = precision_recall_fscore_support(preds_train,labels_train,average='micro')
                ...
                scores, _ = pack_padded_sequence(scores, label_lengths_sorted, batch_first=True)
                targets, _ = pack_padded_sequence(targets, label_lengths_sorted, batch_first=True)
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(dataloader_val,
                ...
                if swa_params:
                    ...
                    scores, labels_sorted, label_lengths_sorted = decoder_swa(encoder_dict, fc_out, labels, label_lengths)
                    targets = labels_sorted[:, 1:]
                    preds, labels = convert_to_array(scores, target,label_lengths_sorted)
                    ...
                    scores, labels_sorted, label_lengths_sorted = decoder(
                        encoder_out, fc_out, labels, label_lengths)
                    targets = labels_sorted[:, 1:]
                    preds, labels = convert_to_array(scores, targets,label_lengths_sorted)
        ...
        prec, recall, _, _ = precision_recall_fscore_support(preds_all,labels_all,average='macro')
        macro_f1 = 2 * prec * recall / (prec + recall)
        print "MACRO prec %.2f%%, recall %.2f%%, f1 %.2f%%" % (
            recall * 100, prec * 100, macro_f1 * 100)

        prec, recall, f1, _ = precision_recall_fscore_support(preds_all,labels_all,average='micro')
